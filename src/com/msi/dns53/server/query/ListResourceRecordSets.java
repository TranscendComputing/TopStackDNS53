...
public class ListResourceRecordSets extends AbstractHeaderAction<ListResourceRecordSetsResult>{
	private final static Logger logger =  Appctx.getLogger(ListResourceRecordSets.class.getName());
	...
	private ListResourceRecordSetsResult listResourceRecordSets(Session sess, ListResourceRecordSetsRequest request) throws ErrorResponse{
		...
		logger.debug("Table " + tableName + " is found for hosted zone with id, " + request.getHostedZoneId() + ".");
		...
		List<DNS53ResourceRecord> records = sqlaccess.listResourceRecords(sess, request.getHostedZoneId(), 
				request.getStartRecordName(), null, request.getStartRecordType(),
				null, -1, -1);
		...
		ns.setName(zoneName);
		ns.setType("NS");
		Collection<ResourceRecord> nsRecords = new LinkedList<ResourceRecord>();*/
