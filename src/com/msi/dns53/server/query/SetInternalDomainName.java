...
public class SetInternalDomainName extends UnsecuredAction{
	private final static Logger logger =  Appctx.getLogger(SetInternalDomainName.class.getName());
	
	@Override
	public String process0(Session session, HttpServletRequest req,
			HttpServletResponse resp, Map<String, String[]> map)
			throws Exception {
		logger.debug(SetInternalDomainName.class.getName() + " has been called!");
		...
		List<String> zoneInfo = 
		HibernateUtil.withNewSession(new Operation<List<String>>() {
			@Override
			public List<String> ex(final Session s, final Object... args)
					throws Exception {
				return sqlaccess.createHostedZone(s, name, callerRef, comment, system.getId());
			}
		});
		...
		if(hzId.equals("DUPLICATE_NAME")){
			...
			RequestHandler.writeChange(sqlaccess, DNS53Constants.PENDING, (new Date()).toString(), zoneInfo.get(1), "CREATE");
		}
		...
		if(ipAddress != null){
			createARecord(session, hostname, ipAddress, system, sqlaccess, nameDot,	hzId);	
		}else if(cnamePrefix != null){
			createCNAMERecord(session, hostname, cnamePrefix, system, sqlaccess, nameDot, hzId);
		}
		return "DONE";
	}
	...
	public void createARecord(Session session, String hostname,
		...
		boolean recordExists = sqlaccess.recordExists(session, hzId, hostnameDot,
				type, value);
		if(!recordExists){
			logger.debug("Record has not been found; creating a new one for the given hostname.");
			sqlaccess.addResourceRecord(hzId, hostname,	ttl, type, value, system.getId());
			...
			RequestHandler.writeChange(sqlaccess, "INSYNC", (new Date()).toString(), "RRSET", "RRSET");
		}else{
	...
	public void createCNAMERecord(Session session, String hostname,
		...
		if(!recordExists){
			logger.debug("Record has not been found; creating a new one for the given hostname.");
			sqlaccess.addResourceRecord(hzId, cnamePrefix.toLowerCase() + "-" + hostname,	ttl, type, hostname, system.getId());
			DNS53ResourceRecord newRecord = new DNS53ResourceRecord();
			newRecord.setZoneId(hzId);
			newRecord.setZoneName(nameDot);
			newRecord.setName(hostnameDot);
