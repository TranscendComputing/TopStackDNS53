...
public class DNS53IntegrationTest extends AbstractBaseDnsTest {
    ...
    /*
     * private void setupUser(){ Session s = HibernateUtil.getSession();
     * s.beginTransaction(); AccountBean ab = new AccountBean();
     * ab.setName("DNS53_TESTER"); ab.setAccessKey(accessKey);
     * ab.setSecretKey(secretKey); ab.setDefSecurityGroups("default");
     * ab.setDefZone("DNS53_TEST_DEFAULT_ZONE"); s.save(ab);
     * s.getTransaction().commit(); }
     *
     * private void cleanupUser(){ Session s = HibernateUtil.getSession();
     * s.beginTransaction(); AccountBean ab = AccountUtil.readAccount(s,
     * accessKey); s.delete(ab); s.getTransaction().commit(); }
     */

    private String getZoneId(String zoneName) {
        Session s = HibernateUtil.getSession();
        s.beginTransaction();
        DNS53HostedZone hz = HostedZoneEntity.selectHostedZone(s, null,
                zoneName);
        return hz.getId();
    }

    /*
     * @Test public void setupTestUser(){ setupUser(); }
     */
    ...
    @Test
    // (expected = HostedZoneAlreadyExistsException.class)
    ...
    @Test
    // (expected = DelegationSetNotAvailableException.class)
    ...
    @Test
    // (expected = InvalidInputException.class)
    ...
    public void getChange() {
        GetChangeRequest req = new GetChangeRequest(
                "CFF0D1C08442C4584B9D39B1311496393");
    ...
    /*
     * @Test public void cleanupTestUser(){ cleanupUser(); }
     */
}
